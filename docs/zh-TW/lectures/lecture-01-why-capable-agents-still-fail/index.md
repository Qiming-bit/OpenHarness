...
最常見的問題是任務說得不夠完整。你說「加個搜尋功能」，agent 理解的跟你完全不一樣，搜什麼？全文字還是結構化？要不要分頁？要不要標記？你沒說清楚，agent 就自己猜。猜對了是運氣，猜錯了你再改，改的成本比一開始說清楚還高。
...
- **Harness**：模型之外的一切，指令、工具、環境、狀態管理、驗證回饋。不是模型權重的部分，全是 harness。
...
- 別只說「模型不行」。逐層排查五個防禦層：任務沒說清楚、脈絡不足、環境配置有問題、缺少驗證、工作階段之間狀態丟失。十次裡有九次，問題出在這五層之中的某一層。
